...
export default function jointJs(containerId) {
    ...
    return {
        ...
        init() {
            ...
            for (const id in this.$wire.cellLinks) {
                this.addLink(this.$wire.cellLinks[id]);
            }
            recenter()

            // Update graph cells based on map changes
            ...
            // Update graph links based on map changes
            this.$wire.$watch('cellLinks', (value) => {
                const links = Alpine.raw(value)

                // Add new links
                for (const id in links) {
                    if (! graph.getCell(id)) {
                        this.addLink(links[id])
                    }
                }

                // Remove deleted links
                graph.getLinks()
                    .filter(link => links[link.id] === undefined)
                    .forEach(link => graph.removeCell(link))
            })
        ...
        addCell(cell, toGraph = null) {
            ...
            if (graph === targetGraph) {
                const positionDebouncer = debouncer(this.$wire.updatePosition, 1000)
        ...
        addLink(link) {
            new Link({
                id: link.id,
                source: { id: link.source, port: link.port },
        ...
        createPalette() {
            ...
            const releaseNewDrag = (event) => {
                ...
                if (
                    ...
                    this.$wire.addCell(dragCell.attr('riversType'), box.x - current.tx, box.y - current.ty)
                }
